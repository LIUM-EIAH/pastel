...
function pastel_refresh_events($courseid = 0) {
	...
	foreach ( $pastels as $pastel ) {
		// Create a function such as the one below to deal with updating
		// calendar events.
		// pastel_update_events($pastel);
	}
...
/**
 * Is a given scale used by the instance of pastel?
 *
 * This function returns if a scale is being used by one pastel
 ...
 * @param int $scaleid
 *        	ID of the scale
 * @return bool true if the scale is used by the given pastel instance
 */
function pastel_scale_used($pastelid, $scaleid) {
	global $DB;
	
	if ($scaleid and $DB->record_exists ( 'pastel', array (
			'id' => $pastelid,
			'grade' => - $scaleid 
	) )) {
		return true;
...
/**
 * Checks if scale is being used by any instance of pastel.
 *
 * This is used to find out if scale used anywhere.
 *
 * @param int $scaleid
 *        	ID of the scale
 * @return boolean true if the scale is used by any pastel instance
 */
function pastel_scale_used_anywhere($scaleid) {
	global $DB;
...
/**
 * Creates or updates grade item for the given pastel instance
 *
 ...
 * @return void
 */
function pastel_grade_item_update(stdClass $pastel, $reset = false) {
	global $CFG;
	require_once ($CFG->libdir . '/gradelib.php');
	
	$item = array ();
	$item ['itemname'] = clean_param ( $pastel->name, PARAM_NOTAGS );
	$item ['gradetype'] = GRADE_TYPE_VALUE;
	
	if ($pastel->grade > 0) {
		$item ['gradetype'] = GRADE_TYPE_VALUE;
		$item ['grademax'] = $pastel->grade;
		$item ['grademin'] = 0;
	} else if ($pastel->grade < 0) {
		$item ['gradetype'] = GRADE_TYPE_SCALE;
		$item ['scaleid'] = - $pastel->grade;
	} else {
	...
	grade_update ( 'mod/pastel', $pastel->course, 'mod', 'pastel', $pastel->id, 0, null, $item );
}

/**
 * Delete grade item for given pastel instance
 *
 * @param stdClass $pastel
 *        	instance object
 * @return grade_item
 */
function pastel_grade_item_delete($pastel) {
	global $CFG;
	require_once ($CFG->libdir . '/gradelib.php');
	
	return grade_update ( 'mod/pastel', $pastel->course, 'mod', 'pastel', $pastel->id, 0, null, array (
			'deleted' => 1 
	) );
}

/**
 * Update pastel grades in the gradebook
 *
 * Needed by {@link grade_update_mod_grades()}.
 *
 * @param stdClass $pastel
 ...
 */
function pastel_update_grades(stdClass $pastel, $userid = 0) {
	global $CFG, $DB;
	require_once ($CFG->libdir . '/gradelib.php');
	...
	grade_update ( 'mod/pastel', $pastel->course, 'mod', 'pastel', $pastel->id, 0, $grades );
}
 ...
 */
function pastel_get_file_areas($course, $cm, $context) {
	return array ();
}

/**
 * File browsing support for pastel file areas
 *
 * @package mod_pastel
 * @category files
 *          
 ...
 */
function pastel_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
	return null;
}

/**
 * Serves the files from the pastel file areas
 *
 * @package mod_pastel
 * @category files
 *          
 ...
 */
function pastel_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options = array()) {
	global $DB, $CFG;
...
/**
 * Extends the global navigation tree by adding pastel nodes if there is a
 * relevant content
 *
 * This can be called by an AJAX request so do not rely on $PAGE as it might not
 * be set up properly.
 *
 * @param navigation_node $navref
 *        	An object representing the navigation tree node of the pastel
 *        	module instance
 * @param stdClass $course
 *        	current course record
 * @param stdClass $module
 *        	current pastel instance record
 * @param cm_info $cm
 *        	course module information
 */
function pastel_extend_navigation(navigation_node $navref, stdClass $course, stdClass $module, cm_info $cm) {
	// TODO Delete this function and its docblock, or implement it.
}

/**
 * Extends the settings navigation with the pastel settings
 *
 * This function is called when the context for the page is a pastel module.
 * This is not called by AJAX
 * so it is safe to rely on the $PAGE.
 *
 * @param settings_navigation $settingsnav
 *        	complete settings navigation tree
 * @param navigation_node $pastelnode
 *        	pastel administration node
 */
function pastel_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $pastelnode = null) {
	// TODO Delete this function and its docblock, or implement it.
}
